...
% attach variables 
CD = linspace(0.045, 1.3, 100); % Drag coefficient
...
% Create a matrix that accounts for the change in distance and height travelled with respect to the coefficient of drag
dataCD_MaxDist = ones(length(CD), 1);
dataCD_MaxHeight= ones(length(CD), 1);

% Get data readings
for i = 1:length(CD)
    const.CD = CD(i);
    [~, state] = ode45(@(t, statevector) rocket_phases(t, statevector, const), tspan, statevector_0);
    dataCD_MaxDist(i) = max(state(:, 1));
    dataCD_MaxHeight(i) = max(state(:, 3));
end

%% Plot dataCD vs CD
figure;
plot(dataCD_MaxDist, CD, 'Color', '[0.4940, 0.1840, 0.5560]', 'LineWidth', 2, LineStyle='-')
hold on;

% Define x value at which the vertical line is needed
requiredDistance = 92;
% Interpolate the y-value (CD) for x = 92
y_interp = interp1(dataCD_MaxDist, CD, requiredDistance, 'linear');
...
correspondingHeight = interp1(dataCD_MaxDist, dataCD_MaxHeight, requiredDistance, 'linear');
correspondingCD= y_interp;

% Display Results
fprintf("Required Max Distance: %2.1f ", requiredDistance);
fprintf("\n\nPredicted Corresponding Coefficient of Drag: %2.3f ", correspondingCD);
fprintf("\nPredicted Corresponding Max Height: %2.1f ", correspondingHeight);

% plots
xlabel('Distance (m)');
ylabel('Coefficient of Drag (Cd)');
title("Change in Distance Travelled vs Coefficient of Drag");
xlim([40 130])
ylim([0 1.5])
xticks([50 75 92 110])
yticks([0.045, y_interp, 0.295, 0.425, 0.47, 1.2])
yline(0.045, '--', 'Color', 'blue', Label={'Standard Airfoil'});
yline(y_interp, '--', 'Color', [0.4660, 0.6740, 0.1880], Label={'Interpolated Value'});
yline(0.295, '--', 'Color', 'blue', Label={'Bullet'});
yline(0.425, '--', 'Color', 'red', Label={'Original Constant'});
yline(0.47, '--', 'Color', 'blue', Label={'Half Sphere'});
yline(1.2, '--', 'Color', 'blue', Label={'Flat Plate'});
grid on;
hold off;
...
%% Plot Trajectory and Thrust Profile With The Predicted Value of Coefficient of Drag at 0.228

% Run ODE with predicted CD = 0.228
const.CD = y_interp; % CD prediction 
[t, state] = ode45(@(t, statevector) rocket_phases(t, statevector, const), tspan, statevector_0); 

Thrust = zeros(length(t), 1);
for i = 1:length(t)
    const.CD = y_interp; % change the coefficient of drag to the predicted value of CD = 0.228
...
fprintf("\nMax Thrust: %2.1f", max(Thrust));


% Trajectory Plot
figure;
plot(distanceVerify, heigthVerify, 'g-', 'LineWidth', 4)
hold on;
...
% Thrust vs Time Plot
figure;
plot(timeVerify, thrustVerify, 'g-', 'LineWidth', 4);
hold on;
...
legend("Verification Data", "Current Data", Location="east");
grid on;
grid minor;
hold off; 
snapnow;
...
function const = getConst()
    ...
    const.p0 = (48+ 12.1)*(6894.76); % Initial air pressure (Pa)
